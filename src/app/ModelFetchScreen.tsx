...
const ModelFetchScreen = () => {
  ...
  const handleBarCodeScanned = async (data: string) => {
    ...
    try {
      ...
      // Check if the URL ends with a valid 3D model extension (GLB only)
      const validExtensions = ['.glb'];
      const hasValidExtension = validExtensions.some(ext => modelUrl.toLowerCase().endsWith(ext));
      
      if (!hasValidExtension) {
        throw new Error('Invalid model format. Please scan a QR code that links to a .glb file.');
      }
  ...
  const navigateToARScreen = async (modelUri: string, source: string) => {
    try {
      ...
      router.push({
        pathname: "/(resources)/ViroARScreen",
        params: { 
          modelUri,
          source,
  ...
  const fetchAvailableModels = async () => {
    ...
    try {
      ...
      // Filter models to only show those with category_id 58 (3D models) AND .glb URLs
      const filtered = fetchedModels.filter(item => item.category_id === 58 && typeof item.url === 'string' && item.url.toLowerCase().endsWith('.glb'));
